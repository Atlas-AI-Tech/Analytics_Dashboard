...
const LatencyTracker = ({ flowId: initialFlowId = '' }) => {
  const [trackingMode, setTrackingMode] = useState('flow_uuid'); // 'flow_uuid' or 'median'
  ...
  const [timeUnit, setTimeUnit] = useState('seconds'); // 'seconds' or 'milliseconds'
  ...
  // Convert time value based on current unit
  ...
  const fetchMedians = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          payload?.error || 'Failed to fetch median metrics',
        );
      }
  ...
  const tableHeaders = [
    ...
    { key: 'fileUploadTime', label: `Upload (${timeUnitLabel})`, tooltip: 'Time between file upload initiation and completion' },
    { key: 'grayScalingTime', label: `Gray Scaling (${timeUnitLabel})`, tooltip: 'Image preprocessing duration' },
    { key: 'sageAPITime', label: `Sage API (${timeUnitLabel})`, tooltip: 'External Sage API processing time' },
    { key: 'filePixelConversion', label: `Pixel Conversion (${timeUnitLabel})`, tooltip: 'Time taken to convert document pixels' },
    { key: 'reduct', label: `Reduct (${timeUnitLabel})`, tooltip: 'Document reduction pipeline duration' },
    { key: 'lentraAuth', label: `Lentra Auth (${timeUnitLabel})`, tooltip: 'Authentication call to Lentra services' },
    { key: 'callbackSuccess', label: `Callback (${timeUnitLabel})`, tooltip: 'Callback acknowledgement time' },
    { key: 'miscellaneous', label: `Misc (${timeUnitLabel})`, tooltip: 'Other processing tasks' },
